...
timezone_kb = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="MSK+1", callback_data="tz_1"), InlineKeyboardButton(text="MSK+2", callback_data="tz_2")],
...
async def save_subscription(user_id, sub_type, periodicity, weekday, day_of_month, time_obj, timezone_offset):
    ...
    logging.info(f"Saving subscription for user {user_id}: sub_type={sub_type}, periodicity={periodicity}, "
                 f"weekday={weekday}, day_of_month={day_of_month}, time={time_obj}")

    conn = await asyncpg.connect(**DB_CONFIG)
    try:
        # Вместо проверки на существующую подписку, просто добавляем новую подписку для этого пользователя
        await conn.execute(''' 
