...
async def start_command(message: types.Message):
    keyboard = types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="PDF", callback_data="revenue_analysis_pdf")],
        [types.InlineKeyboardButton(text="Excel", callback_data="revenue_analysis_excel")]
    ])
    await message.answer("Выберите формат отчёта:", reply_markup=keyboard)

# Обработчик для PDF
@dp.callback_query(F.data == "revenue_analysis_pdf")
async def handle_revenue_analysis_pdf(callback_query: CallbackQuery):
    await callback_query.answer("Формирую PDF отчёт...")
    await handle_format_pdf(callback_query)

# Обработчик для Excel
@dp.callback_query(F.data == "revenue_analysis_excel")
async def handle_revenue_analysis_excel(callback_query: CallbackQuery):
    await callback_query.answer("Формирую Excel отчёт...")
    await handle_format_excel(callback_query)
